                    ...
                    <div class="panel-title">
                        ...
                        <div class="pull-right no-margin">
                            ...
                            <a class="btn btn-primary"
                                href="{{ action('MembersController@edit',['id' => $member->id]) }}">
                                <span>Edit</span>
                            </a>
                            @endpermission
                            ...
                            <button class="btn btn-danger" data-toggle="modal"
                                data-target="#deleteModal-{{$member->id}}" data-id="{{$member->id}}">
                                <span>Delete</span>
                            </button>
                            @endpermission
...
$profileImage = ($images->isEmpty() ? 'https://placeholdit.imgix.net/~text?txtsize=22&txt=NA&w=200&h=180' : url($images[0]->getUrl()));
                                    ...
                                    <div class="col-sm-8"> <!-- Outer Row Start -->
                                        ...
                                        <div class="row">
                                            ...
                                            <div class="col-sm-8">
                                                <span
                                                    class="show-data">{{$member->created_at->toFormattedDateString()}}</span>
                                            </div>
                                        </div>
                                        <hr class="margin-top-0 margin-bottom-10">
                                        <div class="row">
                                        ...
                                        <div class="row">
                                            ...
                                            <div class="col-sm-8">
                                                <span class="show-data">{{$member->note}}</span>
                                            </div>
                                        </div>
...
foreach ($subscriptions as $subscription) {
    $plansArray[] = $subscription->plan->plan_name;
                                                ...
                                                <div class="row">
                                                    ...
                                                    <div class="col-sm-8">
                                                        <span
                                                            class="show-data">{{Utilities::getStatusValue($member->status)}}</span>
                                                    </div>
                                                </div>
                                                <hr class="margin-top-0 margin-bottom-10">
                                                <div class="row">
                                                    ...
                                                    <div class="col-sm-8">
                                                        <span
                                                            class="show-data">{{Utilities::getAim($member->aim)}}</span>
                                                    </div>
                                                </div>
                                                <hr class="margin-top-0 margin-bottom-10">
                                                <div class="row">
                                                ...
                                                <div class="row">
                                                    ...
                                                    <div class="col-sm-8">
                                                        <span class="show-data">{{$pending_amount}}</span>
                                                    </div>
                                                </div>
                            ...
                            <tbody>
                                @foreach ($member->subscriptions->sortByDesc('created_at') as $subscription)
                                    <tr>
                                        <td>
                                            <a
                                                href="{{ action('InvoicesController@show',['id' => $subscription->invoice_id]) }}">{{ $subscription->invoice->invoice_number }}</a>
                                        </td>
                                        ...
                                        <td>{{$subscription->invoice->pending_amount }}</td>
                                    </tr>
                                @endforeach
                            </tbody>
                        </table>
        ...
        <!--######################### purchase history for the member ################################# -->
        <div class="row">
            <div class="col-md-12">
                <div class="panel no-border ">
                    <div class="panel-title">
                        <div class="panel-head font-size-20">Purchase history for the member</div>
                    </div>
                    <div class="panel-body">
                        <table id="_payment" class="table table-bordered table-striped">
                            <thead>
                            ...
                            <tbody>
                                @foreach ($member->purchases->sortByDesc('created_at') as $purchase)
                                    <tr>
                                        <td>
                                            <a
                                                href="{{ action('InvoicesController@show',['id' => $purchase->invoice_id]) }}">{{ $purchase->invoice->invoice_number }}</a>
                                        </td>
                                        <td>{{ $purchase->product->product_name }}</td>

                                        <td>{{ Utilities::getInvoiceStatus($purchase->invoice->status) }}</td>
                                        <td>{{ $purchase->invoice->pending_amount }}</td>
                                    </tr>
                                @endforeach
                            </tbody>
                        </table>
