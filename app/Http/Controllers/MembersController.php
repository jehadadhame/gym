...
class MembersController extends Controller
{
    ...
    public function active(Request $request)
    {
        $members = Member::active(
            isset($request->sort_field) ? $request->sort_field : 'created_at',
            isset($request->sort_direction) ? $request->sort_direction : 'desc',
            $request->drp_start,
            $request->drp_end,
            $request->input('search')
        )->paginate(100);
    ...
    public function show($id)
    {
        ...
        return view('members.show', compact('member', 'pending_amount'));
    }
    ...
    {
        $member = Member::where("contact", "==", $phoneNumber);
        $invoices = $member->invoices;
        $pending_amount = 0;
        foreach ($invoices as $invoice) {
            $pending_amount += $invoice->pending_amount;
