	...
	return {
		...
		purchases: function () {
			function getProductAmount() {
				var sum = 0;
				$(".product-id select option:selected").each(function () {
					sum += +$(this).data('price');
					$('.product-price input#price_' + $(this).data('row-id')).val($(this).data('price'));
				});
				$("#purchase_amount").val(sum).trigger('change');
			}

			function getTaxAmount() {
				var purchase_amount = parseInt($("#purchase_amount").val()); // Get Selected plan amount
				var additional_fees = $("#additional_fees").length ? parseInt($("#additional_fees").val()) : 0; // Get selected admission amount
				var total_amount = purchase_amount + additional_fees;
				var tax = Math.round(total_amount * taxes / 100);  // Calculate taxes
			...
			function getDiscountAmount() {
				var purchase = parseInt($("#purchase_amount").val()); // Get Selected plan amount
				var additional_fees = $("#additional_fees").length ? parseInt($("#additional_fees").val()) : 0; // Get selected admission amount
				var total_amount = purchase + additional_fees;
				var tax = Math.round(total_amount * taxes / 100);  // Calculate taxes
				var total = total_amount + tax; // Calculate total and prefill in payment_amount box
			...
			// On purchase amount Change tax & discount amount
			$("#purchase_amount").bind('change keyup input', function (e) {
				getTaxAmount();
				getDiscountAmount();
			});
			...
			$('#addPurchase').click(function () {
				if (x < max_fields) {
					...
					// Clone base field set and clean it
					$("#servicesContainer>.row:first-child").clone().appendTo("#servicesContainer"); // Add new product purchase
					$("#servicesContainer>.row:last-child .remove-service").removeClass("hide"); // Active remove button for additional services
					$("#servicesContainer>.row:last-child .bootstrap-select").remove(); // remove bootstrap select
					...
					// Set Unique Ids and names
					$("#servicesContainer>.row:last-child .product-id>select").attr("id", "product_" + x).attr("name", "product[" + x + "][id]").attr('data-row-id', x);
					$("#servicesContainer>.row:last-child .product-id>select>option").attr('data-row-id', x);
					$("#servicesContainer>.row:last-child .product-price>input").attr("id", "price_" + x).attr("name", "product[" + x + "][price]").attr('data-row-id', x);

					// Adding Validators
					// $('#membersform').bootstrapValidator('addField', 'product[' + x + '].start_date', startDateValidators);
